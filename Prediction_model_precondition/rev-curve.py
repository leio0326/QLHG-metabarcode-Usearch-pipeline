...
def plot_comparison(results_df, true_plant1_value, true_plant2_value):
    ...
    ax1.errorbar(valid_results['sample_size'], valid_results['plant1_mean_pred'],
                 ...
                 label=f'Plant1 (x={valid_results["insect_target_x"].iloc[0]})')
    ax1.errorbar(valid_results['sample_size'], valid_results['plant2_mean_pred'],
                 yerr=valid_results['plant2_std_pred'], fmt='s-', capsize=5,
                 label=f'Plant2 (x={valid_results["insect_target_x"].iloc[0]})')
    ax1.axhline(true_plant1_value, color='r', linestyle='--', label=f'True Plant1 ({true_plant1_value})')
    ax1.axhline(true_plant2_value, color='g', linestyle='--', label=f'True Plant2 ({true_plant2_value})')
    ax1.set_xlabel("Sample size")
    ax1.set_ylabel("Predicted plant richness")
    ax1.legend()
    ax1.grid(True)
...
if __name__ == "__main__":
    ...
    plant1, plant2, insect, samples, plant1_target, plant2_target, true_insect_richness = load_data(
        ...
        f"{input_dir}/May/all/new_May-feed.csv",  # 新增的plant2数据
        f"{input_dir}/May/all/May-ins.csv"
    )
    ...
    if not results[results['n_valid'] > 0].empty:
        fig = plot_comparison(results, plant1_target, plant2_target)
        fig.savefig(f"{output_dir}/May_rev_results_plot.png",
                    dpi=300, bbox_inches='tight')
        print("Comparison plot saved to insect_to_plants_comparison_plot.png")
    else:
